...
class MyDatePicker(tk.Toplevel):
    ...
    def get_date(self, clicked=None):
        ...
        # Set in value
        self.widget.delete(0, 100)
        self.widget.insert(0, self.full_date)
        xl_runner(self.widget)
...
def parseToDate(text):
    return datetime.datetime.strptime(text, '%d-%B-%Y')


def xl_runner(widget):
    filename = askopenfilename(defaultextension='.xlsm',
                               filetypes=[('Excel with macro', '*.xlsm'), ('Excel', '*.xls')])
    book = xlwings.Book(filename)
    sheet = book.sheets(3)
    currentdate = parseToDate(widget.get())
    start_x = 3
    start_y = 3
    i = 0
    macro = book.macro("cbCalculate_Click")
    # Run always
    sheet.range(start_y, start_x + 1).value = None
    if sheet.range(start_y, start_x + 1).value is None:
        macro()
        while sheet.range(start_y, start_x + 1).value is None:
            i = 0

    while sheet.range(start_y, start_x + i).value is not None:
        addweek = sheet.range(start_y, start_x + i).value
        if i == 0:
            addweek -= 1
        currentdate += datetime.timedelta(days=(7 * addweek))
        sheet.range(start_y, start_x + i).value = currentdate.strftime("%d.%m.%Y")
        sheet.range(start_y, start_x + i).options(dates=datetime.date).value
        i += 1
    book.save()
...
if __name__ == '__main__':
    def application(master):
        MyDatePicker(master, format_str='%02d-%s-%s')
